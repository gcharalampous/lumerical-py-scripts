...
from DEVICE.disk_modulator.dc_sweep.voltage_sweep import voltage_sweep
...
if(__name__=="__main__"):
    with lumapi.DEVICE() as device:
        ...
        plt.semilogy(V_a,(R*1e-3)/(device_length*norm_length),'-')
        ...
        # xmin = min(V_a)
        # xmax = max(V_a)
        # plt.hlines(Cj*1e15*1e-6, xmin=xmin, xmax=xmax, color='r', linestyle='--', 
        #         label='Theoretical')

        # # Extend y-axis to include both simulated and theoretical values
        # y_min = min(min(C_static*1e15*1e-6), Cj*1e15*1e-6) * 0.5  # Add some margin
        # y_max = max(max(C_static*1e15*1e-6), Cj*1e15*1e-6) * 2.0   # Add some margin
        ...
        y_data = device_length * C_static * 1e15
        y_min = np.min(y_data)
        y_max = np.max(y_data)
        plt.semilogy(V_a, y_data, '-', label='Cn')
