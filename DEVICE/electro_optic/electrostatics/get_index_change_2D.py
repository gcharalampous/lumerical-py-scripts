...
if __name__ == "__main__":
    ...
    with lumapi.DEVICE(hide=False) as device:
        ...
        if slab_thickness > 0:
            filter1 = ((X < simulation_span_x / 2) & (X > wg_width / 2)) | (
                (X > -simulation_span_x / 2) & (X < -wg_width / 2)
            )
        ...
        plt.pcolormesh(
            X * 1e6,
            Z * 1e6,
            np.transpose(abs(Dnz)),
            shading="gouraud",
            cmap="jet",
            norm=LogNorm(vmin=v_min, vmax=v_max),
        )
        cbar = plt.colorbar()
        cbar.set_label(r"$\Delta n_z$")
        plt.xlabel("x [um]")
        plt.ylabel("z [um]")
        ...
        plt.pcolormesh(
            X * 1e6,
            Z * 1e6,
            np.transpose(abs(Dnx)),
            shading="gouraud",
            cmap="jet",
            norm=LogNorm(vmin=1e-7, vmax=v_max),
        )
        cbar = plt.colorbar()
        cbar.set_label(r"$\Delta n_x$")
        plt.xlabel("x [um]")
        plt.ylabel("z [um]")
