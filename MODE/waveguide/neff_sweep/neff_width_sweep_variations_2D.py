...
if __name__ == "__main__":
    with lumapi.MODE() as mode:
        ...
        for m in range(1, num_modes + 1):
            ...
            plt.semilogy(
                wg_width_array * 1e6, np.real(dneffdwidth) * 1e-9, "-o", label="M-" + str(m)
            )

        plt.legend()
        plt.ylim([1e-6, 1e-2])
        plt.xlabel("waveguide width (um)")
        plt.ylabel(r"$d(n_{eff})/d(w)\quad (nm^{-1})$")
        plt.title("thickness " + str(wg_thickness * 1e6) + " um")

        # Save the figure files as .png

        file_name_plot = str(out["figure_groups"]["Neff Sweep"] / "neff_width_sweep_variations.png")
