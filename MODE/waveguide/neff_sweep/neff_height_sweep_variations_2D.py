...
if __name__ == "__main__":
    with lumapi.MODE() as mode:
        ...
        for m in range(1, num_modes + 1):
            ...
            plt.semilogy(
                wg_height_array * 1e6, np.real(dneffdwidth) * 1e-9, "-o", label="M-" + str(m)
            )

        plt.legend()
        plt.ylim([1e-5, 1e-2])
        plt.xlabel("height (um)")
        plt.ylabel(r"$\partial(n_{eff})/\partial(h)\quad (nm^{-1})$")
        ...
        file_name_plot = str(
            out["figure_groups"]["Neff Sweep"] / "neff_height_sweep_variations.png"
        )
