...
if __name__ == "__main__":
    with lumapi.MODE() as mode:
        ...
        plt.pcolormesh(
            x * 1e6, y * 1e6, np.real(np.transpose(index_x)), shading="gouraud", cmap="jet"
        )
        plt.colorbar()
        ...
        plt.subplots(figsize=(512 * px, 256 * px))
        plt.pcolormesh(
            x * 1e6, y * 1e6, np.imag(np.transpose(index_x)), shading="gouraud", cmap="jet"
        )
        plt.colorbar()
        ...
        for i in range(1, 3):
            ...
            if pcm_layer_enable:
                ...
                bpcm = sg.box(
                    ...
                    (wg_thickness) * 1e6,
                    pcm_max * 1e6,
                    (wg_thickness + pcm_thickness) * 1e6,
                )
