...
def radiusSweep(mode):
    ...
    for r in range(0, len(wg_radius_array)):
        ...
        for m in range(1, num_modes + 1):
            ...
            polariz_frac[r][m - 1] = mode.getdata(
                "FDE::data::mode" + str(m), "TE polarization fraction"
            )
            ...
            if polariz_frac[r][m - 1] > 0.5:  # identify the TE-like or TM-like modes
                if track_mode == "TE":
                    ...
                    mode.save(
                        str(out["lumerical"] / ("waveguide_mode_radius_sweep_" + str(r) + ".lms"))
                    )
                    overlap_TE.append(
                        mode.overlap("mode_TE_radius_0", "mode_TE" + "_radius_" + str(r))
                    )
                    print(overlap_TE)
                    break
            else:
                if track_mode == "TM":
                    ...
                    overlap_TM.append(
                        mode.overlap("mode_TM_radius_0", "mode_TM" + "_radius_" + str(r))
                    )
                    print(overlap_TM)
                    break
...
if __name__ == "__main__":
    with lumapi.MODE() as mode:
        ...
        if track_mode == "TE":
            ...
            radiation_loss_TE_dB = (
                (np.squeeze(np.abs(loss_TE)) - np.abs(loss_TE[0]))
                * 0.5
                * np.pi
                * wg_radius_array.reshape(1, N)
            )
            ...
            for i in range(0, len(wg_radius_array)):
                ...
                # Total Loss
            total_loss_TE_dB = (
                np.dot(1, overlap_TE_array_dB)
                + np.dot(1, radiation_loss_TE_dB)
                + np.dot(1, PropagationLoss_dB)
            )

            plt.semilogy(
                wg_radius_array[1:N] * 1e6, overlap_TE_array_dB[1:N], "-", label="Mode-missmatch"
            )
            plt.semilogy(
                wg_radius_array[1:N] * 1e6, radiation_loss_TE_dB[1:N], "-", label="Radiation Loss"
            )
            plt.semilogy(
                wg_radius_array[1:N] * 1e6,
                PropagationLoss_dB[1:N],
                "-",
                label=str(PropagationLoss_dB_cm) + " dB/cm",
            )
            plt.semilogy(
                wg_radius_array[1:N] * 1e6, total_loss_TE_dB[1:N], "--", label="Total Loss"
            )
            plt.legend()
        else:
            # Radiation Loss [dB] per quarter

            radiation_loss_TM_dB = np.abs(
                (np.squeeze(loss_TM) - loss_TM[0]) * 0.5 * np.pi * wg_radius_array.reshape(1, N)
            )
            ...
            for i in range(0, len(wg_radius_array)):
                ...
                # Total Loss
            total_loss_TM_dB = (
                np.dot(1, overlap_TM_array_dB)
                + np.dot(1, radiation_loss_TM_dB)
                + np.dot(1, PropagationLoss_dB)
            )

            plt.semilogy(
                wg_radius_array[1:N] * 1e6, overlap_TM_array_dB[1:N], "-o", label="Mode-missmatch"
            )
            plt.semilogy(
                wg_radius_array[1:N] * 1e6, radiation_loss_TM_dB[1:N], "-o", label="Radiation Loss"
            )
            plt.semilogy(
                wg_radius_array[1:N] * 1e6, PropagationLoss_dB[1:N], "-o", label="Propagation Loss"
            )
            plt.semilogy(
                wg_radius_array[1:N] * 1e6, total_loss_TM_dB[1:N], "--", label="Total Loss"
            )
            plt.legend()
        plt.tight_layout()
