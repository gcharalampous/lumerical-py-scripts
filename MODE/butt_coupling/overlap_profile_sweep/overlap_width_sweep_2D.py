    ...
    with lumapi.MODE() as mode:
        if polarization_wg_1 == "TE":
            ...
            for wd in range(0, len(wg_2_width_array)):
                ...
                mode.mesh()
                mode.findmodes()
                file_name_mode_writing = str(
                    lumerical_dir / ("waveguide_2_mode_sweep_" + str(wd) + ".lms")
                )
                mode.save(file_name_mode_writing)

                file_name_data_writing[wd] = str(
                    lumerical_dir / ("waveguide_2_data_sweep_" + str(wd) + ".ldf")
                )
                ...
                for m in range(1, num_modes + 1):
                    ...
                    if polariz_frac[m - 1] > 0.5:  # identify the TE-like or TM-like modes
                        ...
                        overlap_TE[wd] = mode.overlap(
                            "waveguide_1_mode" + str(m_waveguide1), "waveguide_2_mode" + str(m)
                        )
