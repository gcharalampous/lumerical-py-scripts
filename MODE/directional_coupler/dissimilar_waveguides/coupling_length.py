...
if __name__ == "__main__":
    ...
    with lumapi.MODE(str(templates[0])) as mode:
        ...
        Power_WGA = (
            c1_sq**2 + c2_sq**2 + 2 * c1_sq * c2_sq * np.cos(delta_beta * coupling_beat_length)
        )
        ...
        plt.plot(
            coupling_beat_length,
            10 * np.log10(Power_WGA),
            coupling_beat_length,
            10 * np.log10(Power_WGB),
        )
        ...
        # Save the plot
        file_name_plot = str(
            out["figure_groups"]["Dissimilar Waveguides"] / "length_TE_dissimilar_waveguides.png"
        )
