...
# ---------------------------------------------------------------------------
"""
Extract and plot transmission response for adiabatic directional coupler.

The script plots the transmission for the adiabatic directional coupler
structure defined in the sbend_adiabatic_directional_coupler.fsp file,
extracting data from the 'through' and 'bar' monitor ports.
"""
...
# Analysis parameters
TARGET_3DB = -3.01  # Target 3dB point for analysis
...
def getCrossResponse(fdtd):
    ...
    Returns:
        tuple: (T1, T2, f) where
            - T1: Through port transmission
            - T2: Bar port transmission
            - f: Frequency array
    """
...
if __name__ == "__main__":
    with lumapi.FDTD(str(template_fsp)) as fdtd:
        ...
        ax.plot(wavelength, T1, label='Through')
        ax.plot(wavelength, abs(T2), label='Coupled', linestyle='--')
        ax.set_ylim(0, 1)
        ax.grid(which='both')
        ax.legend()
        ...
        ax.plot(wavelength, T1_dB, label='Through')
        ax.plot(wavelength, T2_dB, label='Coupled', linestyle='--')
        ax.grid(which='major')
        ax.legend()
        ax.set_xlabel("Wavelength (µm)")
