...
def getCouplingResponse(fdtd):
    ...
    wavelength = np.squeeze(
        fdtd.getsweepresult("sweep_fiber_position_x", "transmission").get("lambda")
    )
    position_x = np.squeeze(
        fdtd.getsweepresult("sweep_fiber_position_x", "transmission").get("fiber_position_x")
    )
    ...
    try:
        ...
        with lumapi.FDTD(str(template_fsp)) as fdtd:
            ...
            plot_max_transmission_at_wavelength(
                T, wavelength, position_x, target_wavelength, file_name_plot
            )
