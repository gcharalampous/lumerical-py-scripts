...
# --------------------------------Plot-T/C---------------------------------
        ...
        ax.plot(length*1e6, bar, label = 'Bar')
        ax.grid(which='both')
        ax.legend()
        ...
        plt.tight_layout()
        file_name_plot = os.path.join(FDTD_ADIAB_Y_BR_DIRECTORY_WRITE[1], "sweep_length_linear.png")
        plt.savefig(file_name_plot)        
        
        px = 1/plt.rcParams['figure.dpi']  # pixel in inches
        fig, ax = plt.subplots(figsize=(512*px, 256*px))
        ax.plot(length*1e6, 10 * np.log10(through), label = 'Through')
        ax.plot(length*1e6, 10 * np.log10(bar), label = 'Bar')
        ax.grid(which='both')
        ax.legend()
        ax.set_xlabel("length (μm)")
        ax.set_ylabel("Magnitude")
        ax.set_title("wavelength[μm] = {:.3f}".format(lambda0*1e6))
        plt.tight_layout()
        file_name_plot = os.path.join(FDTD_ADIAB_Y_BR_DIRECTORY_WRITE[1], "sweep_length_log.png")
        plt.savefig(file_name_plot)        
