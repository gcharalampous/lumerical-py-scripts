...
def getFields(fdtd):
    ...
    return (y, Ey_lin, z, Ez_lin, x_xy, y_xy, E_xy, x_xz, z_xz, E_xz, y_yz, z_yz, E_yz)
...
if __name__ == "__main__":
    with lumapi.FDTD(str(template_fsp)) as fdtd:
        ...
        (y, Ey_lin, z, Ez_lin, x_xy, y_xy, E_xy, x_xz, z_xz, E_xz, y_yz, z_yz, E_yz) = getFields(
            fdtd
        )
        ...
        # ---- Top-view (xy) ----
        fig, ax = plt.subplots(figsize=(512 * px, 256 * px))
        ax.pcolormesh(x_xy * 1e6, y_xy * 1e6, np.transpose(E_xy), shading="gouraud", cmap="jet")
        ax.set_xlabel("x (\u00b5m)")
        ax.set_ylabel("y (\u00b5m)")
        ...
        # ---- Side-view (xz) ----
        fig, ax = plt.subplots(figsize=(512 * px, 256 * px))
        ax.pcolormesh(x_xz * 1e6, z_xz * 1e6, np.transpose(E_xz), shading="gouraud", cmap="jet")
        ax.set_xlabel("x (\u00b5m)")
        ax.set_ylabel("z (\u00b5m)")
        ...
        # ---- Cross-section (yz) ----
        fig, ax = plt.subplots(figsize=(512 * px, 256 * px))
        ax.pcolormesh(y_yz * 1e6, z_yz * 1e6, np.transpose(E_yz), shading="gouraud", cmap="jet")
        ax.set_xlabel("y (\u00b5m)")
        ax.set_ylabel("z (\u00b5m)")
